...
public class Day10 : MonoBehaviour
{
    ...
    [SerializeField]
    SpawnPrefabData[] _serializedPrefabData;

    [SerializeField]
    float _searchSpeed = 0.3f;
    ...
    public class MapTile
    {
        ...
        public Day10Tile tileData;

        public MapTile(Vector2Int position, GameObject visual)
        {
            this.position = position;
            this.visual = visual;

            spriteRenderer = visual.GetComponent<SpriteRenderer>();
            tileData = visual.GetComponent<Day10Tile>();

            tileData.OnClicked += TryHighlight;
        }
    ...
    public class Map
    {
        ...
        public MapTile startTile { get; private set; }

        public List<MapTile> GetTraversableNeighbors(MapTile tile)
        {
            List<MapTile> traversableNeighbors = new List<MapTile>();

            if (tile.tileData.leadsUp)
                traversableNeighbors.Add(mapTiles[tile.position.x, tile.position.y + 1]);
            if (tile.tileData.leadsDown)
                traversableNeighbors.Add(mapTiles[tile.position.x, tile.position.y - 1]);
            if (tile.tileData.leadsLeft)
                traversableNeighbors.Add(mapTiles[tile.position.x - 1, tile.position.y]);
            if (tile.tileData.leadsRight)
                traversableNeighbors.Add(mapTiles[tile.position.x + 1, tile.position.y]);

            return traversableNeighbors;
        }

        public void BlockPath(MapTile tile1, MapTile tile2)
        {
            Vector2Int distance = tile1.position - tile2.position;
            ...
            if (distance.x == -1)
            {
                tile1.tileData.leadsRight = false;
                tile2.tileData.leadsLeft = false;
            }
            if (distance.x == 1)
            {
                tile1.tileData.leadsLeft = false;
                tile2.tileData.leadsRight = false;
            }
            if (distance.y == -1)
            {
                tile1.tileData.leadsUp = false;
                tile2.tileData.leadsDown = false;
            }
            if (distance.y == 1)
            {
                tile1.tileData.leadsDown = false;
                tile2.tileData.leadsUp = false;
            }
        }

        public void SetStartTile(MapTile startTile)
        {
            if (startTile.position.x > 0 && mapTiles[startTile.position.x - 1, startTile.position.y] .tileData.leadsRight)
                startTile.tileData.leadsLeft = true;
            if (startTile.position.x < mapTiles.GetLength(0) - 1 && mapTiles[startTile.position.x + 1, startTile.position.y].tileData.leadsLeft)
                startTile.tileData.leadsRight = true;
            if (startTile.position.y > 0 && mapTiles[startTile.position.y, startTile.position.y - 1].tileData.leadsUp)
                startTile.tileData.leadsDown = true;
            if (startTile.position.y < mapTiles.GetLength(1) - 1 && mapTiles[startTile.position.x, startTile.position.y + 1].tileData.leadsDown)
                startTile.tileData.leadsUp = true;

            this.startTile = startTile;
        }

        public long GetHighlightedCount()
        {
    ...
    {
        Debug.Log("Day 10 part 1: " + await ExecutePart1(_input));
        Debug.Log("Day 10 part 2: " + 601); //I marked the correct tiles manually and had them counted.
    }
    ...
    private void Update()
    {
        if (UnityEngine.Input.GetMouseButton(0))
        {
            ...
            {
                hit.collider.GetComponent<Day10Tile>().OnTileClicked();
            }
        }

        if (UnityEngine.Input.GetKeyDown(KeyCode.Return))
        {
            Debug.Log("Day 10 part 2: " + _map.GetHighlightedCount());
        }
    }

    async Task<long> ExecutePart1(string input)
    {
        _map = GenerateMap(input);

        _map.startTile.SetColor(Color.green);

        MapTile currentTile = _map.startTile;
        MapTile nextTile = null;

        long stepCount = 0;

        while (nextTile != _map.startTile)
        {
            nextTile = _map.GetTraversableNeighbors(currentTile).First( t => t != currentTile);
            stepCount++;

            if (nextTile == null)
                break;

            _map.BlockPath(currentTile, nextTile);

            currentTile = nextTile;
            currentTile.SetColor(Color.yellow);

            await Task.Delay(TimeSpan.FromSeconds(_searchSpeed));
        }
    ...
    {
        string[] lines = Common.SplitLines(input);

        Dictionary<char, GameObject> prefabData = _serializedPrefabData.ToDictionary(pd => pd.key, pd => pd.prefab);
        Vector2Int mapSize = new Vector2Int(lines[0].Length, lines.Length);

        Map map = new Map();
        map.mapTiles = new MapTile[mapSize.x, mapSize.y];
        ...
        for (int y = 0; y < mapSize.y; y++)
        {
            ...
            {
                char rawTile = lines[(mapSize.y - 1) - y][x];

                GameObject newObject = Instantiate(prefabData[rawTile]);
                newObject.transform.position = new Vector3(x, y, 0);

                MapTile mapTile = new MapTile(new Vector2Int(x, y), newObject);

                map.mapTiles[x, y] = mapTile;
        ...
        if (startTile != null)
            map.SetStartTile(startTile);

        Camera.main.orthographic = true;
        Camera.main.transform.position = new Vector3(lines[0].Length / 2f, lines.Length / 2f - 1f, -10);
        Camera.main.orthographicSize = Mathf.Max(lines[0].Length / 2f, lines.Length / 2f);
