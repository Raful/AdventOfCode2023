...
public class Day19 : MonoBehaviour
{
    ...
    void Awake()
    {
        Debug.Log("Day 19 part 1: " + Execute(_input));
    }

    public static long Execute(string input)
    {
        //Pseudo code:
        //workflow = GetWorkflows["in"];
        //string workflowOutput = part.ParseInput("in", workflow)
        //if (workflowOutput == "A")
        //    accepted
        //else if (workflowOutput == "R")
        //    rejected
        //else
        //    workflowOutput = part.ParseInput(workflowOutput, workflow)
        //repeat

        string[] splitInput = input.Split("\r\n\r\n");

        Part[] parts = GetParts(splitInput[1]);
        Dictionary<string, Workflow> workflows = GetWorkflows(splitInput[0]);
        ...
        foreach (Part part in parts)
        {
            ...
            {
                workflowLabel = workflows[workflowLabel].GetNextWorkflow(part);
            }
    ...
    public static Part[] GetParts(string partsInput)
    {
        //{x=787,m=2655,a=1222,s=2876}\r\n{x=1679,m=44,a=2067,s=496}

        string[] lines = Common.SplitLines(partsInput);

        Part[] parts = new Part[lines.Length];

        for (int i = 0; i < lines.Length; i++)
        {
            parts[i] = new Part(lines[i]);
        }

        return parts;
    }

    public static Dictionary<string, Workflow> GetWorkflows(string workflowInput)
    {
        //px{a<2006:qkq,m>2090:A,rfg}\r\npv{a>1716:R,A}
        string[] lines = Common.SplitLines(workflowInput);
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            workflows.Add(workflow.label, workflow);
        }
    ...
    public class Workflow
    {
        public class Statement
        {
            public Statement(string rawStatement)
            {
                ...
                compareInput = rawStatement[0];
                compareBiggerThan = rawStatement[1] == '>';
                compareTo = int.Parse(rawStatement.Substring(2, rawStatement.IndexOf(':') - 2));

                trueOuput = rawStatement.Substring(rawStatement.IndexOf(':') + 1, rawStatement.IndexOf(',') - rawStatement.IndexOf(':') - 1);
                falseOuput = rawStatement.Substring(rawStatement.IndexOf(',') + 1, rawStatement.Length - rawStatement.IndexOf(',') - 1);
            }

            public char compareInput;
            public bool compareBiggerThan;
            public long compareTo;

            public string trueOuput;
            public string falseOuput;

            public string GetOutputFromPart(Part part)
            {
                long partValue;
                switch (compareInput)
                {
                    ...
                    default:
                        throw new Exception("Unexpected part value: " + compareInput);
                }

                if (compareBiggerThan)
                    return partValue > compareTo ? trueOuput : falseOuput;
                else
                    return partValue < compareTo ? trueOuput : falseOuput;
            }
        }
        ...
        public Workflow(string input)
        {
            ...
            label = input.Substring(0, input.IndexOf('{'));

            statement = new Statement(input.Substring(input.IndexOf('{') + 1, input.IndexOf('}') - input.IndexOf('{') - 1));
        }
        ...
        public string GetNextWorkflow(Part part)
        {
            ...
            {
                output = currentStatement.GetOutputFromPart(part);

                if (output.Contains(":"))
                    currentStatement = new Statement(output);
                else
                    return output;
            }
    ...
    public class Part
    {
        public Part(string rawInput)
        {
            //{x=787,m=2655,a=1222,s=2876}

            string[] splitInput = rawInput.Trim(new char[] { '{', '}' }).Split(',');

            x = int.Parse(splitInput[0].Substring(2, splitInput[0].Length - 2));
            m = int.Parse(splitInput[1].Substring(2, splitInput[1].Length - 2));
            a = int.Parse(splitInput[2].Substring(2, splitInput[2].Length - 2));
            s = int.Parse(splitInput[3].Substring(2, splitInput[3].Length - 2));
        }
